<div class="sidebar" id="sidebar">
    ...
    @if (auth()->check() && auth()->user()->can_see_reports)
        <h4> @icon(bar - chart) {{ trans_choice('report.report', 2) }}</h4>
        <ul>
            @include('components.sidebarItem', [
                'url' => route('reports.index'),
                'title' => trans_choice('report.report', 2),
            ])
            @include('components.sidebarItem', [
                'url' => route('reports.analytics'),
                'title' => trans_choice('report.analytics', 1),
            ])
        </ul>
    @endif
    ...
    <ul>
        @include('components.sidebarItem', [
            ...
            'title' => trans_choice('team.team', 2),
        ])
        @if (auth()->user()->admin)
            @include('components.sidebarItem', [
                'url' => route('users.index'),
                'title' => trans_choice('ticket.user', 2),
            ])
            @include('components.sidebarItem', [
            ...
            @include('components.sidebarItem', [
                'url' => route('ticketTypes.index', 1),
                'title' => trans_choice('ticket.ticketType', 2),
            ])
        @endif
    </ul>
    <br>
